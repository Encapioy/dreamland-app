            ...
            <div class="bg-white rounded-3xl shadow-xl overflow-hidden transform transition-all hover:scale-[1.01]">
                ...
                <div class="{{ $headerColor }} p-5 text-white">
                    ...
                    @if($order->customer_phone)
                        @php
            // Logic Smart WhatsApp
            $rawPhone = $order->customer_phone;
            $waPhone = preg_replace('/^0/', '62', $rawPhone);
            $waName = $order->customer_name;
            $waLoc = $order->location;

            $msg = match ($category) {
                'guru' => "Assalamu'alaikum Ustadz/Ustadzah $waName, mohon maaf mengganggu. Pesanan antum di stand Takis Station sudah siap ($waLoc). Syukron.",
                'walsan' => "Assalamu'alaikum Bapak/Ibu $waName. Pesanan Anda di stand Takis Station sudah siap ($waLoc). Terima kasih.",
                default => "Halo $waName, pesananmu di Takis Station sudah siap nih ($waLoc). Silakan diambil ya!",
            };
